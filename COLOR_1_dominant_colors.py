...
##############################################################################
# specify,  (1) directory path where positive tiles are 
#           (2) output filename (feature color values) 
dir_pos = './landuse_construction/color_dist'
featured_color_filename ='./landuse_construction.txt'


HEXs_Freq = color_dist(dir_pos, descending=True)
rgb_list = hex_to_rgb(HEXs_Freq, n_most_rgb=10)
feature_colors = dominant_color_set(rgb_list, n_most = 1, rgb_buffers=(5, 5, 5))

min_R, min_G, min_B = feature_colors[0][0]
max_R, max_G, max_B = feature_colors[0][1]


f = open(featured_color_filename, "a")
print(min_R, file=f)
print(max_R, file=f)
print(min_G, file=f)
print(max_G, file=f)
...
#fr = open(featured_color_filename, "r")
#col_vals = fr.read().splitlines()
#fr.close()
#
